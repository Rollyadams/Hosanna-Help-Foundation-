...
function ChatWindow({ visitor, convoId }) {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.new.sender_id !== visitor.id) {
          // Mark this message and any other unread ones
          supabase.from('hhf_messages')
            .select('id')
            .eq('conversation_id', convoId)
            .neq('sender_id', visitor.id)
            .neq('status', 'read')
            .then(({ data: unreadMsgs }) => {
  ...
  async function loadMessages() {
    ...
    if (data) {
      ...
      const unread = enriched.filter(m => m.sender_id !== visitor.id && m.status !== 'read').map(m => m.id)
      if (unread.length > 0) {
